...
data_dir=r"data/LabPicsV1/Simple/" # Path to dataset (LabPics 1)
os.mkdir("model/")
saved_model = f"checkpoints/model/sam2_hiera_small_{datetime.datetime.now().strftime('%Y-%m-%d')_"simple"}.torch"

# Read data
data=[] # list of files in dataset
for ff, name in tqdm(enumerate(os.listdir(data_dir+"Train/Image/")), desc="Reading data"):  # go over all folder annotation
    data.append({
         "image":data_dir+"Train/Image/"+name,
         "annotation":data_dir+"Train/Instance/"+name[:-4]+".png"
     })
...
sam2_model = build_sam2(
     model_cfg, sam2_checkpoint, device="cuda") # load model
...
for itr in tqdm(range(100000), desc="training"):
    with torch.cuda.amp.autocast(): # cast to mix precision
          ...
          if itr%1000==0: torch.save(predictor.model.state_dict(), saved_model);print("save model")
